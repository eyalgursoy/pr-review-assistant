...
- **Reply to host comments**: Reply to PR/MR comments from the IDE. Use **Reply** from the comment thread toolbar, tree view context menu, or the "Reply" CodeLens on host comments. Enter your reply in the prompt; it is posted to GitHub/GitLab/Bitbucket and host comments are refreshed. Only available when a PR/MR is loaded and for comments from the host (not AI-generated).
- **Review Workflow & Collaboration (Task 2)**: Provider APIs for replying to host comments and resolving/unresolving threads. `replyToComment(pr, comment, body)` and `setThreadResolved(pr, threadId, resolved)` added to `PRProvider`; implemented for GitHub (REST reply + GraphQL resolve/unresolve), GitLab (discussion notes + PUT discussion), and Bitbucket (reply with parent id; resolve no-op with success message). Enables Task 3 (Reply UI) and Task 4 (Resolve/Unresolve UI).
